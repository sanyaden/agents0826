# Готовий app/<name>/main.py для проєкту, створеного `agentcore create`
# (Strands · Anthropic · памʼять none · HTTP). Замініть ним скаффолдний
# main.py — і в хмару поїде той самий агент підтримки, що й локально.
# Перевірено 02.09.2026: повторний deploy 52 с, invoke 16 с.
#
...
DEFAULT_SYSTEM_PROMPT = """Ти — агент підтримки поштового оператора. Відповідай українською, коротко. Статус і право на повернення бери ТІЛЬКИ з інструментів; не вигадуй правил. За прострочення повертається лише вартість доставки, не вкладення."""
...
tools.append(check_refund_eligibility)
